...
def reset_paddle():
    ...
    canvas.delete(computer_q)
    canvas.delete(computer_b)

    computer_q = canvas.create_rectangle((SIZE - 10, 240, SIZE, 144), fill="black")
    computer_b = canvas.create_rectangle((0, 240, 10, 144), fill="black")
...
def get_pad_loc():
    ...
    paddle_coords_b = canvas.coords(computer_b)
    paddle_coords_q = canvas.coords(computer_q)

    pad_pos_b = (paddle_coords_b[1] + paddle_coords_b[3]) / 2
    pad_loc_q = int(pad_pos/(SIZE/12))-1
